import { db } from "@/db"
import { ProductFilterValidor } from "@/lib/validators/product-validator"
...
export const POST = async (req: NextRequest) => {
    try {
        ...
        const { color, price, size, sort } = ProductFilterValidor.parse(body.filter);

        const filter = new Filter()

        if (color.length > 0) color.forEach((clr) => filter.add("color", "=", clr))
        else if (color.length === 0) filter.addRaw("color", `color = ""`)


        if (size.length > 0) size.forEach((sz) => filter.add("size", "=", sz))
